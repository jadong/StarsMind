 ...
 */
public class JokeImageFragment extends Fragment implements JokeListView {
    ...
    private void setListener() {
        ...
        swipeRefreshLayout.setOnPullRefreshListener(new SuperSwipeRefreshLayout.OnPullRefreshListener() {
            ...
            public void onRefresh() {
                ...
                if (item != null) {
                    jokePresenter.getJokeByTime("image",item.getUnixtime());
                }
    ...
    private void loadData() {
        jokePresenter.getJokeList("image",pageNo);
    }
    ...
    public void refreshData(boolean isByTime, List<Joke> jokeList) {
        ...
        boolean hasMore = jokeList != null && jokeList.size() >= 10;//判断是否还有下页数据(api没有返回页码,暂时这样实现)
        if (pageNo == 1) {
            if (hasMore) {
